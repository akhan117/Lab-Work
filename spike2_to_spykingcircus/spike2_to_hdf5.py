...
# Written by Ayaan Khan and Ian Lee
# This program creates an HDF5 file combining data from raw Spike2 files, along with data the user want's to enter!
...
if __name__ == "__main__":
    # Checking if a default value for the number of files someone wants to use exists. If it doesn't exist, it will
    # ask the user for how many files to use (usually 2 - 1 for pre infusion, 1 for post infusion) and ask if they
    # want to save that as the default value. Default values are in "Default Values".
    ...
    # Creating the list of files from user input
    for i in range(0, int(x)):
        print("There are still " + str(int(x) - i) + " file(s) to choose")
        val = askopenfilename()
        file_list.append(val)

    # Similarly, this asks where to save the files to only if a default location does not exist.
    ...
    with open("Temporary .txt files/Channels.txt", 'w') as f:
    ...
    events_f_b = []
    # Organize the events by time and save them, for the user perusal
    with open("Temporary .txt files/Events.txt", 'w') as f:
    ...
    # Use are saved events file to create a properly formatted Events and Time List.
    with open("Temporary .txt files/Events.txt", 'r') as f:
    ...
    # Move into parent folder, then move into spike_processing, so we can save information for the future there.
    os.chdir("..")
    events = os.path.abspath(os.curdir)

    if platform.system() == "Windows":
        times = events + "\\spike_processing\\Event and Sampling Rates Data\\" + \
            full_name + " times.pk"
        rates = events + "\\spike_processing\\Event and Sampling Rates Data\\" + \
            full_name + " rates.pk"
        splits = events + "\\spike_processing\\Event and Sampling Rates Data\\" + \
            full_name + " splits.pk"
        odor_onsets = events + "\\spike_processing\\Event and Sampling Rates Data\\" + \
            full_name + " odor_onsets.pk"
        events = events + "\\spike_processing\\Event and Sampling Rates Data\\" + \
            full_name + " events.pk"
    ...
    # We save the Event Timings, the sampling rate, the split (between the files), The Odor onsets, and the Event names.
    # This is saved for Activity Extractor.py Later.
    ...
    # If the user would like to enter Miscellaneous data about the experiment, they may choose to do so here.
    # This will be appended to the file created at the end of this Experiment.
    print(
